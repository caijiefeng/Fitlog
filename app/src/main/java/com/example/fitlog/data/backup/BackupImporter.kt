 ...
 * @property rowsImported  Number of database rows imported.
 ...
 * - **ZIP Slip protection**: every entry name is resolved against the temp
 *   extraction directory and verified to be within it via canonical path.
 ...
 * 2. `db.json` rows are inserted table by table using raw SQL
 *    (`INSERT OR REPLACE`) to bypass Room entity validation during bulk import.
 ...
 *
 * @param db               The Room database whose openHelper is used for raw SQL.
 * @param appMediaStorage  Storage backend for resolving/restoring media files.
 */
class BackupImporter(
     ...
     * 2. Deletes all existing records and inserts new ones in a single Room
     *    transaction (rolled back on failure).
     * 3. Copies media files from the ZIP to [AppMediaStorage].
     *
     * @throws ImportException on validation or import error.
     */
     ...
     *
     * **ZIP Slip protection**: every entry name is canonicalised and must
     * resolve within the intended output directory.
     *
    ...
    private fun readZipEntries(input: InputStream): Map<String, ByteArray> {
        ...
        val baseDir = File(System.getProperty("java.io.tmpdir"), "fitlog_import_validate").canonicalFile
        ...
        while (entry != null) {
            ...
            if (!entry.isDirectory) {
                ...
                if (!entryFile.startsWith(baseDir)) {
                    throw ImportException(
                        "ZIP Slip detected: entry \"${entry.name}\" would resolve outside the extraction directory.",
                    )
                }
    ...
    private fun importAllTables(dbJson: JSONObject) {
        importTable(dbJson, "exercise_categories", ::insertExerciseCategory)
        importTable(dbJson, "exercises", ::insertExercise)
        importTable(dbJson, "workout_templates", ::insertWorkoutTemplate)
        importTable(dbJson, "workout_template_exercises", ::insertWorkoutTemplateExercise)
        importTable(dbJson, "workout_schedules", ::insertWorkoutSchedule)
        importTable(dbJson, "workout_sessions", ::insertWorkoutSession)
        importTable(dbJson, "exercise_sessions", ::insertExerciseSession)
        importTable(dbJson, "set_records", ::insertSetRecord)
        importTable(dbJson, "workout_plan_overrides", ::insertWorkoutPlanOverride)
        importTable(dbJson, "reminders", ::insertReminder)
        importTable(dbJson, "check_ins", ::insertCheckIn)
        importTable(dbJson, "user_profiles", ::insertUserProfile)
        importTable(dbJson, "body_measurements", ::insertBodyMeasurement)
        importTable(dbJson, "food_records", ::insertFoodRecord)
        importTable(dbJson, "media_records", ::insertMediaRecord)
    }
    ...
    companion object {
        /** Versions of backup format this importer can handle. */
